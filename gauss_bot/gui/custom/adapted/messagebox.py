...
Modified by: Joaquín Zúñiga, on 11/9/2024
...
class CustomMessagebox(ctkTop):
    def __init__(
        ...
        sound: bool = False,
        wraplength: int = 0,
        option_focus: Optional[Literal[1, 2, 3]] = None,
    ):
        ...
        self.icon = self.load_icon(icon) if icon else None
        ...
        if self.message:
            self.info._text_label.configure(wraplength=self.width / 2, justify="left")  # type: ignore
            self.info.grid(
                row=1, column=0, columnspan=6, sticky="nwes", padx=self.border_width
            )

            if wraplength > 0:
                self.info._text_label.configure(wraplength=wraplength)  # type: ignore

            if self.info._text_label.winfo_reqheight() > self.height / 2:  # type: ignore
                height_offset = int(
                    (self.info._text_label.winfo_reqheight())  # type: ignore
                    - (self.height / 2)
                    + self.height
                )
        ...
        if option_2:
            self.button_2 = ctkButton(
                self.frame_top,
                text=self.option_text_2 if self.option_text_2 else "",
        ...
        if option_3:
            self.button_3 = ctkButton(
                self.frame_top,
                text=self.option_text_3 if self.option_text_3 else "",
